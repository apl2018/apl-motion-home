...
## ✅ 완료 (2026-06-22)
- **Apple 개발자 프로그램 가입 완료** — 조직(주식회사 에이피엘) 심사 승인 → 라이선스 동의 → 결제 완료. 주문번호 W1550143595. (계정 활성화 안내는 gmail로 수신)
- 사이트 https 정상(Enforce HTTPS 켜짐). 홈페이지·도메인·이메일·Apple 가입 = 전부 완료.

## ⏭️ 다음(앱 출시 트랙 — 별도 진행) 
APL Motion 폴더의 「출시준비_업종코드_진행계획」 트랙 B 참고:
1. 앱 스토어용 네이티브 래핑(Capacitor 등) + Apple 결제(IAP) 연동
2. Xcode 빌드·실기기 테스트(Mac 필요) → App Store Connect 등록 → 심사 제출
3. 개인정보처리방침·라이선스 고지·의료표현 점검(심사 전 필수)
